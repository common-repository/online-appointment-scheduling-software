 ...
 */
if (!function_exists('scal_dashboard_callback')) {

    function scal_dashboard_callback() {

        include_once( ABSPATH . WPINC . '/feed.php' );
        wp_enqueue_script('swift-form-jstz', SWIFTCAL__PLUGIN_URL . 'admin/js/jstz.min.js', '', '', true);

        wp_enqueue_style('swift-dashboard', SWIFTCAL__PLUGIN_URL . 'admin/css/swift-dashboard.css', '', '', '');
        wp_enqueue_script(SWIFTCAL_PLUGIN_PREFIX . 'dashboard-script', SWIFTCAL__PLUGIN_URL . 'admin/js/swift-dashboard.js', array('jquery'), '', true);
        wp_enqueue_style('swiftcloud-fontawesome', "//maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css", '', '', '');

        $subscribed = get_option(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe');
        $subs_checkbox_flag = isset($_COOKIE[SWIFTCAL_PLUGIN_PREFIX . 'dashboard_unsubscribe']) && $_COOKIE[SWIFTCAL_PLUGIN_PREFIX . 'dashboard_unsubscribe'] == 1 ? "block" : "none";
        ...
        <div class="wrap dashboard-wrap">
            ...
            <?php } ?>
            <!--subscribe box-->
            <div class="dashboard-subscribe-block" id="scal_subscribe" style="display: <?php echo $subs_box_flag; ?>;">
                ...
                <form method="post" id="frm_scal_dashboard_subscribe">
                    ...
                    <div class="dashboard-col-8 col-left">
                        <div class="col-dashboard-block">
                            ...
                            <div class="col-dashboard-block-content">
                                <?php
                                $rss = fetch_feed('https://SwiftCloud.AI/support/tag/calendar/feed'); //Change here
                                $maxitems = 0;
 ...
 *      cookie value : 0 || 1
 */
add_action('wp_ajax_' . SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
add_action('wp_ajax_nopriv_' . SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox', SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback');
if (!function_exists(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox_callback')) {

    function scal_dashboard_subscribe_checkbox_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_checkbox') {
            setcookie(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTCAL_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTCAL_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTCAL_PLUGIN_PREFIX . 'dashboard_close_subscribe', SWIFTCAL_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback');
if (!function_exists(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_close_subscribe_callback')) {

    function scal_dashboard_close_subscribe_callback() {
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTCAL_PLUGIN_PREFIX . 'dashboard_close_subscribe') {
            setcookie(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_unsubscribe', "1", time() + (10 * 365 * 24 * 60 * 60), "/", '');
            echo "1";
        }
        wp_die();
    }
...
add_action('wp_ajax_' . SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_callback');
add_action('wp_ajax_nopriv_' . SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe', SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_callback');

if (!function_exists(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe_callback')) {

    function scal_dashboard_subscribe_callback() {
        check_ajax_referer('swiftdashboard_subs_form', 'swiftdashboard_subs_form');
        if (isset($_POST['action']) && !empty($_POST['action']) && $_POST['action'] == SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe') {
            parse_str($_POST['data'], $subscribe_form_data);

            update_option(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_subscribe', true);
            setcookie(SWIFTCAL_PLUGIN_PREFIX . 'dashboard_unsubscribe', "", time() - 1, "/", '');
